...
st.markdown('App to predict an Iris flower, take some measurment on your flower')
...
with st.form('iris_data'):
    ...
    sepal_width = st.number_input('' \
        'Sepal length (cm)', min_value=1.5 , max_value=4.9, value=3.0
        )
    petal_length = st.number_input('' \
        'Sepal length (cm)', min_value=0.5 , max_value=7.4, value=4.0
        )
    petal_width = st.number_input('' \
        'Sepal length (cm)', min_value=0.1 , max_value=3.0, value=2.0
        )

    submitted = st.form_submit_button('PREDICT')

if submitted:
    payload = {'sepal_length': sepal_length,
